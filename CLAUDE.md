...
- **Runtime API config:** the SPA discovers the API URL at runtime by fetching `/config.json` (deployed alongside the SPA assets by `core_stack._api`, with `apiBaseUrl` = the HTTP API URL substituted at deploy time via a Token-aware `s3deploy.Source.data`). Build artifacts are **environment-agnostic** — the same `dashboard/dist` runs against any deployed stack; there is **no** build-time `VITE_*` URL injection.
- **Lambda bundling contract (guard):** a `PythonLambda` with a full-dotted handler (e.g. `dashboard_api.handler.handler`) **must** set `include_shared=True` — that layout places the package dir + `shared/` at the zip root so the handler and `from <pkg>... import` resolve. `include_shared=False` flattens the entry's contents to the root and requires the `handler.handler` form. `test_synth_constraints.test_every_python_lambda_handler_resolves_in_bundle` enforces this at synth time so a mismatch fails the build instead of cold-starting with `Runtime.ImportModuleError`.
- **Adding a new route:** add a handler in `lambdas/dashboard_api/routes/` and register it in the `ROUTES` dict in `handler.py`; add the route to the HTTP API in `core_stack._api` (it inherits the bearer authorizer automatically). On the frontend, add a `src/pages/` view and wire its URL into the `react-router-dom` `Routes` in `App.tsx`. Route handlers return a plain object for 200 or a `(body, status_code)` tuple for non-200s.

### Approval flow (Phase 9b)

Findings with `policy_decision=prompt` + `status=open` are eligible for human review via the dashboard. The finding **detail** page (never the list) renders Approve / Reject buttons; the mutating endpoint is `POST /approvals/{approval_id}` on the dashboard API. This is the framework's first **destructive user surface** — Approve runs the matching remediation runbook against the resource.

- **Actor identity:** the first action prompts for the operator's name, stored in `localStorage` as `tidewater_approver` and sent as `approver`. It is **self-attested** for audit attribution — the bearer token is what authenticates; the name is not verified identity.
- **`approval_id` is derived, not looked up:** the dashboard computes it client-side from the finding identity (`lib/approval.ts`, SHA-256, must mirror `policy_engine.handler.approval_id_for`), so the URL is predictable.
- **Approve** invokes the remediator Lambda **async** (`InvocationType=Event`), exactly as the policy engine does for auto decisions; the finding flips to `in_remediation` immediately and the SPA refetches. Failures surface via the Phase 8 `remediation.failed` → SNS email channel — a sync call would give illusory confidence since the runbook runs for minutes afterward.
- **Reject** closes **this** finding only (`status=skipped`); it does **not** permanently exclude the resource. If the rule trips again a fresh finding fires. Permanent exclusion is the `tidewater-skip: true` tag applied to the AWS resource. Reject takes an optional ≤200-char reason.
- **Concurrent decisions are safe:** the approval row uses a conditional `UpdateItem` on `status = pending`; the second decider gets a **409** with the existing decision, and (on approve) the remediator is never invoked twice.
- Decisions emit `Approval.granted` / `Approval.denied` EventBridge events and `approval_granted` / `approval_rejected` JSONL audit records to `s3://<audit-bucket>/audit/`.
