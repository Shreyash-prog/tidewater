...
class CoreStack(Stack):
    def __init__(
        ...
        **kwargs: Any,
        ...
        api_url, dashboard_fn = self._api(
            distribution, deploy_version, tables=tables, buckets=buckets, bus=bus
        )
    ...
    def _api(
        ...
        *,
        ...
        dashboard_fn = PythonLambda(
            ...
            environment={
                "FINDINGS_TABLE": findings_table.table_name,
                "APPROVALS_TABLE": tables["approvals"].table_name,
                "RULES_BUCKET": buckets["rules-yaml"].bucket_name,
                "AUDIT_LOG_BUCKET": buckets["audit-log"].bucket_name,
                # write_audit_event reads AUDIT_BUCKET (the audit-log bucket).
                "AUDIT_BUCKET": buckets["audit-log"].bucket_name,
                "SNAPSHOTS_BUCKET": buckets["snapshots"].bucket_name,
                "EVENT_BUS_NAME": bus.event_bus_name,
            },
            description="Dashboard API: read-only findings/rules + approve/reject. Phase 9a/9b.",
        )
        dfn = dashboard_fn.function
        # Reads everywhere; the only writes are the approval-decision path (9b):
        # the approval row + the finding's status, and the audit record in S3.
        findings_table.grant(dfn, "dynamodb:Scan", "dynamodb:GetItem", "dynamodb:UpdateItem")
        tables["approvals"].grant(dfn, "dynamodb:GetItem", "dynamodb:UpdateItem")
        buckets["rules-yaml"].grant_read(dfn)
        buckets["audit-log"].grant_read(dfn)
        buckets["audit-log"].grant_put(dfn)
        buckets["snapshots"].grant_read(dfn)
        bus.grant_put_events_to(dfn)
        ...
        # The one mutating route (Phase 9b): approve/reject a finding.
        http_api.add_routes(
            path="/approvals/{approval_id}", methods=[HttpMethod.POST], integration=integration
        )
        ...
        return http_api.api_endpoint, dfn
    ...
    def _policy_and_remediation(
        ...
        *,
        ...
        # Phase 9b: the dashboard API invokes the remediator (async) on approval,
        # the same way the policy engine does for auto decisions. Wired here because
        # the remediator function only exists after _api() created the dashboard fn.
        dashboard_fn.add_environment("REMEDIATOR_FUNCTION_NAME", rfn.function_name)
        rfn.grant_invoke(dashboard_fn)
